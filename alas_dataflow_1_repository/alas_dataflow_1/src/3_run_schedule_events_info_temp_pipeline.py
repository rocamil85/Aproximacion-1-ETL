...
def process_schedule_events_info(record):
    ...
    if 'schedule_events_info_json' not in record or record['schedule_events_info_json'] is None:
        return

    events = json.loads(record['schedule_events_info_json'])
    for event in events:
...
def run_schedule_events_info_temp_pipeline(file_list):
    pipeline_options = get_pipeline_options()

    # Define aquí el esquema de la tabla events_info_temp como una cadena
    schema_schedule_events_info_temp_str = ','.join([
        "timestamp:TIMESTAMP",
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        for file_path in file_list:
            ...
            (p
             ...
             | f'ExtraerYTransformarEventos_{base_name}' >> beam.FlatMap(process_schedule_events_info)
             | f'EscribirABigQueryEventsInfo_{base_name}' >> beam.io.WriteToBigQuery(
                 f'{PROJECT_ID}:{DATASET}.schedule_events_info_temp',
                 schema=schema_schedule_events_info_temp_str,
...
if __name__ == '__main__':
    ...
    logging.info("Iniciando el pipeline de Beam 3_run_schedule_events_info_temp_pipeline...")
    ...
    logging.info("Inicia el pipeline 3_run_schedule_events_info_temp_pipeline...")
    run_schedule_events_info_temp_pipeline(file_list)
        
    logging.info("============   Fin de 3_run_schedule_events_info_temp_pipeline   ============")
